...
def run_experiment(config_name_arg: str, model_path_arg: str, num_episodes: int, max_steps: int, render: bool):
    ...
    if os.path.isdir(model_path_arg):
        ...
        algo_model_dir_key = temp_config.algorithm.lower()
        models_dir_from_default_config = f"models/{algo_model_dir_key}/" 
        constructed_path = os.path.join(models_dir_from_default_config, model_path_arg)
    ...
    if experiment_dir_of_model and os.path.exists(os.path.join(experiment_dir_of_model, "config.json")):
        ...
        print(f"Warning: config.json not found. Using config '{config_name_arg}' directly.")
    ...
    # SAC agent needs SACConfig, WorldConfig, and TrainingConfig
    agent = SAC(config=config.sac, world_config=config.world, training_config=config.training, device=device)
    print(f"Loading SAC model weights from {os.path.abspath(actual_model_file_path)}...")
    agent.load_model(actual_model_file_path)

    print(f"\nRunning experiment with SAC model {os.path.basename(actual_model_file_path)}...")
...
if __name__ == "__main__":
    ...
    parser.add_argument("--config", "-c", type=str, default="default", help="Fallback config if config.json not found. Avail: " + f"{list(CONFIGS.keys())}")
    parser.add_argument("--model", "-m", type=str, required=True, help="Path to SAC model checkpoint (.pt) OR experiment directory.")
